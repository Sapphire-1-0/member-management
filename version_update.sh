...
# Extract the version below <artifactId>member-management</artifactId> and trim spaces
CURRENT_VERSION=$(awk '
    /<artifactId>member-management<\/artifactId>/ { found=1 }
    found && /<version>/ {
...
if [[ -z "$CURRENT_VERSION" ]]; then
    echo "Error: Unable to find <version> tag for <artifactId>member-management</artifactId> in pom.xml"
    exit 1
fi
